...
MQTT::Client.connect(ENV['MQTT_HOST'], ENV['MQTT_PORT'].to_i) do |c|
    ...
    c.get(ENV['MQTT_TOP'])  do |topic,message|
      ...
      result = client.query('SELECT alarm_power, alarm_on FROM sonoff.pow ORDER BY idpow DESC LIMIT 1')
      result = client.query("INSERT INTO sonoff.pow (datetime, power, factor, voltage, current, alarm_power, alarm_on) VALUES 
